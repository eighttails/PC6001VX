...
互換ROM使用時は初代PC-6001、PC-6001A、PC-6001mk2、PC-6601いずれかに相当する動作になります。PC-6001mk2SR、PC-6601SRも選択可能ですが、BASICモード6には未対応です。モード5までのソフトをバスリクエストOFFによる高速化をしたい場合に選択してください。 +
必要なROMファイルは以下の通りです。 +
[WARNING]
...
* ファイルの共有 +
Androidではアプリのサンドボックス外にファイルを直接書き出すことができません。スナップショットの取得およびTAPE(SAVE)のエクスポートを行うと共有メニューが表示されますので、ファイルマネージャや画像編集アプリなど任意のアプリに送ることができます。 +
...
* キーを押し続けると、実機のキーリピートに近いタイミングで入力を繰り返します。
...
. `bash ./release.sh` を実行。必要なツール(QtCreator, SDL2, asciidoctor)の導入、スタティック版Qt/FFmpegの構築、CMakeによるReleaseビルド、配布フォルダ作成までまとめて行います。 +
スタティック版Qtは `/ucrt64/local/qt6-static-private` に配置されます。 +
. `release.sh` の中では `buildrelease.sh` を使って、ルートの `CMakeLists.txt` を Ninja + CMake でReleaseビルドしています。配布用EXEは `PC6001VX_release/PC6001VX_(バージョン)_win_64bit` に出力されます。 +
. 自分でソースを修正、デバッグをする場合は、ルートにあるCMakeLists.txtをQtCreatorで開いて、MSYS2 UCRT64 + スタティック版Qtを使うキットでビルドしてください。キットの構成方法は本書の範囲外とします。 +
...
. シェルにて `apt install build-essential cmake ninja-build libx11-dev libsdl2-dev qtcreator qt6-base-dev qt6-multimedia-dev qt6-declarative-dev libavformat-dev libavcodec-dev libswscale-dev` を実行。 +
. ダウンロードしたPC6001VXのソースを解凍してください。 +
. QtCreatorでルートのCMakeLists.txtを開いてビルドしてください。 +
