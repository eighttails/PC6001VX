/*
  bdf2cgrom68.c  2026.4.28 version
    convert .bdf font file to CGROM68.68 format for PC-6001mkIISR/6601SR
    by AKIKAWA, Hisashi
...
void convert(FILE *fp, unsigned int bmp60[16], int size_x_out, int size_y_out, int pixel_x, int pixel_y, int over_baseline, int code_is_ambiguous, double par1, double par2, double par3, double par4);
int isnodata(unsigned int *bmp60);
void calcline(unsigned int bmp60[][16], int size_y_out);
...
int main(int argc, char *argv[])
{
  ...
  unsigned int bmp_8x12[0x100][0x10] = {0};
  unsigned int bmp_8x10[0x200][0x10] = {0};
  unsigned int bmp_8x8[0x100][0x10] = {0};
  unsigned int bmp_16x16[0x100][0x10] = {0};
  unsigned int bmpmix[0x100][0x10] = {0};
  unsigned int bmptmp[0x10] = {0};
  long fpos;
  FILE *fpin, *fpout;
  FILE *fp_8x12, *fp_8x10, *fp_8x8, *fp_16x16;
  ...
  const int (*(kanjitbl[]))[2] = {jiskanjitbl, unikanjitbl};

  /* options */
  fp_8x12 = fp_8x10 = fp_8x8 = fp_16x16 = NULL;
  ...
  for (opt = 1; argc > opt && argv[opt][0] == '-'; opt++) {
    if (strcmp(argv[opt], "-ascii") == 0) {
      ...
      use_ascent = 1;
    } else if (strcmp(argv[opt], "-12") == 0 && ++opt < argc) {
      if (strcmp(argv[opt], "-") == 0) {
	fp_8x12 = stdin;
      } else {
	fp_8x12 = fopen(argv[opt], "rb");
	if (fp_8x12 == NULL) {
	  printf("cannot open %s\n", argv[opt]);
	  exit(1);
	}
      }
    } else if (strcmp(argv[opt], "-10") == 0 && ++opt < argc) {
      if (strcmp(argv[opt], "-") == 0) {
	fp_8x10 = stdin;
      } else {
	fp_8x10 = fopen(argv[opt], "rb");
	if (fp_8x10 == NULL) {
	  printf("cannot open %s\n", argv[opt]);
	  exit(1);
	}
      }
    } else if (strcmp(argv[opt], "-8") == 0 && ++opt < argc) {
      if (strcmp(argv[opt], "-") == 0) {
	fp_8x8 = stdin;
      } else {
	fp_8x8 = fopen(argv[opt], "rb");
	if (fp_8x8 == NULL) {
	  printf("cannot open %s\n", argv[opt]);
	  exit(1);
	}
      }
    } else if (strcmp(argv[opt], "-16") == 0 && ++opt < argc) {
      if (strcmp(argv[opt], "-") == 0) {
	fp_16x16 = stdin;
      } else {
	fp_16x16 = fopen(argv[opt], "rb");
	if (fp_16x16 == NULL) {
	  printf("cannot open %s\n", argv[opt]);
	  exit(1);
	}
    ...
    printf("usage: bdf2cgrom68 [-ascii] [-ascent] [-12 font_8x12.bdf] [-10 font_8x10.bdf [-8 font_8x8.bdf] [-16 font_16x16.bdf] [font.bdf]\n");
      exit(1);
    }
  }

  if (opt == argc - 1) {
    fpin = fopen(argv[opt], "rb");
    if (fpin == NULL) {
      printf("cannot open %s\n", argv[opt]);
      exit(1);
    }
  } else if (opt == argc) {
    if (fp_8x12 == NULL || fp_8x10 == NULL || fp_8x8 == NULL || fp_16x16 == NULL) {
      printf("usage: bdf2cgrom68 [-ascii] [-ascent] [-12 font_8x12.bdf] [-10 font_8x10.bdf [-8 font_8x8.bdf] [-16 font_16x16.bdf] [font.bdf]\n");
      exit(1);
    }
  } else {
  ...
  /* 8x12 font */
  charset = getbdfpar(fp_8x12, ascii, use_ascent, &pixel_x, &pixel_y, &over_baseline);
  while (fgets(line, sizeof(line), fp_8x12)) {
	...
	convert(fp_8x12, bmp_8x12[i], 8, 12, pixel_x, pixel_y, over_baseline, code_is_ambiguous, 1, 0, 1, 0);
	fseek(fp_8x12, fpos, SEEK_SET);
      }
    }
	...
	if (isnodata(bmp_8x12[chr])) {
	  convert(fp_8x12, bmp_8x12[chr], 8, 12, pixel_x, pixel_y, over_baseline, code_is_ambiguous, 1, 0, 1, 0);
	  fseek(fp_8x12, fpos, SEEK_SET);
	}
      }
    }
  ...
  /* box drawings */
  calcline(bmp_8x12, 12);

  rewind(fp_8x12);

  /* 8x8 font */
  charset = getbdfpar(fp_8x8, ascii, use_ascent, &pixel_x, &pixel_y, &over_baseline);
  while (fgets(line, sizeof(line), fp_8x8)) {
    if (sscanf(line, "ENCODING %d", &code) != 1 || code == 0) continue;
    fpos = ftell(fp_8x8);
    code_is_ambiguous = isambiguous(code, charset);
    for (i = 0; i < 0x100; i++) {
      if ((ascii && code == i) || (!ascii && code == tbl[charset][i])) {
	convert(fp_8x8, bmp_8x8[i], 8, 8, pixel_x, pixel_y, over_baseline, code_is_ambiguous, 1, 0, 1, 0);
	fseek(fp_8x8, fpos, SEEK_SET);
      }
    }
    ...
    for (i = 0; i < sizeof(jisalttbl) / sizeof(jisalttbl[0][0]) / 2; i++) {
      if (alttbl[charset][i][0] < 0x100 && code == alttbl[charset][i][1]) {
	chr = alttbl[charset][i][0];
	if (isnodata(bmp_8x8[chr])) {
	  convert(fp_8x8, bmp_8x8[chr], 8, 8, pixel_x, pixel_y, over_baseline, code_is_ambiguous, 1, 0, 1, 0);
	  fseek(fp_8x8, fpos, SEEK_SET);
	}
      }
    }
  ...
  /* 8x10 font */
  charset = getbdfpar(fp_8x10, ascii, use_ascent, &pixel_x, &pixel_y, &over_baseline);
  while (fgets(line, sizeof(line), fp_8x10)) {
    if (sscanf(line, "ENCODING %d", &code) != 1 || code == 0) continue;
    fpos = ftell(fp_8x10);
    code_is_ambiguous = isambiguous(code, charset);
    for (i = 0; i < 0x200; i++) {
      if ((ascii && code == i) || (!ascii && code == tbl[charset][i])) {
	convert(fp_8x10, bmp_8x10[i], 8, 10, pixel_x, pixel_y, over_baseline, code_is_ambiguous, 1, 0, 1, 0);
	fseek(fp_8x10, fpos, SEEK_SET);
      }
    }
	...
	chr = alttbl[charset][i][0];
	if (isnodata(bmp_8x10[chr])) {
	  convert(fp_8x10, bmp_8x10[chr], 8, 10, pixel_x, pixel_y, over_baseline, code_is_ambiguous, 1, 0, 1, 0);
	  fseek(fp_8x10, fpos, SEEK_SET);
	}
      }
    }
	...
	if (code == mixtbl[charset][i][1]) {
	  convert(fp_8x10, bmptmp, 8, 10, pixel_x, pixel_y, over_baseline, code_is_ambiguous,
  ...
  for (i = 0x10; i < 0x19; i++) {
    for (j = 0; j < 10; j++) {
      bmp_8x10[0x100 + i][j] = (j >= 0x19 - i) ? 0xff : 0;
    }
  }
  ...
  for (i = 0x19; i < 0x20; i++) {
    for (j = 0; j < 10; j++) {
      bmp_8x10[0x100 + i][j] = (0xfe << (i - 0x19)) & 0xff;
    }
  }

  /* triangle */
  for (j = 0; j < 10; j++) {
    bmp_8x10[0x120][j] = (0x02 << (j * 8 / 10)) - 0x01;
    bmp_8x10[0x121][j] = 0x100 - (0x80 >> (j * 8 / 10));
    bmp_8x10[0x122][j] = (0x100 >> (j * 8 / 10)) - 0x01;
    bmp_8x10[0x123][j] = 0x100 - (0x01 << (j * 8 / 10));
  }

  /* mesh */
  for (j = 0; j < 10; j++) {
    bmp_8x10[0x124][j] = 0xcc >> (j & 2);
    bmp_8x10[0x125][j] = 0xaa >> (j & 1);
  }

  /* mixed character */
  for (i = 0; i < sizeof(jismixtbl) / sizeof(jismixtbl[0][0]) / 3; i++) {
    chr = 0x100 + mixtbl[charset][i][0];
    if (isnodata(bmp_8x10[chr]) && mixdone1[i] && mixdone2[i]) {
      memcpy(bmp_8x10[chr], bmpmix[i], 10);
    }
  }

  /* no data */
  for (i = 0x101; i < 0x200; i++) {
    if (isnodata(bmp_8x10[i])) {
      for (j = 0; j < 10 - 1; j++) {
	bmp_8x10[i][j] = 0xfe;
      }
      bmp_8x10[i][10 / 2 - 1] = 0;
  ...
  /* 16x16 font */
  charset = getbdfpar(fp_16x16, ascii, use_ascent, &pixel_x, &pixel_y, &over_baseline);
  while (fgets(line, sizeof(line), fp_16x16)) {
    if (sscanf(line, "ENCODING %d", &code) != 1 || code == 0) continue;
    fpos = ftell(fp_16x16);
    code_is_ambiguous = isambiguous(code, charset);
    for (i = 0; i < 0x100; i++) {
      for (j = 0; j < sizeof(jiskanjitbl) / sizeof(jiskanjitbl[0][0]) / 2; j++) {
	if (i == kanjitbl[charset][j][0]) break;
      }
      if (j < sizeof(jiskanjitbl) / sizeof(jiskanjitbl[0][0]) / 2) {
	if (code == kanjitbl[charset][j][1]) {
	  convert(fp_16x16, bmp_16x16[i], 16, 16, pixel_x, pixel_y, over_baseline, code_is_ambiguous, 1, 0, 1, 0);
	  fseek(fp_16x16, fpos, SEEK_SET);
	}
      } else if ((ascii && code == i) || (!ascii && code == tbl[charset][i])) {
	convert(fp_16x16, bmp_16x16[i], 16, 16, pixel_x, pixel_y, over_baseline, code_is_ambiguous, 1, 0, 1, 0);
	fseek(fp_16x16, fpos, SEEK_SET);
      }
    }
    ...
    for (i = 0; i < sizeof(jisalttbl) / sizeof(jisalttbl[0][0]) / 2; i++) {
      if (alttbl[charset][i][0] < 0x100 && code == alttbl[charset][i][1]) {
	chr = alttbl[charset][i][0];
	if (isnodata(bmp_16x16[chr])) {
	  convert(fp_16x16, bmp_16x16[chr], 8, 8, pixel_x, pixel_y, over_baseline, code_is_ambiguous, 1, 0, 1, 0);
	  fseek(fp_16x16, fpos, SEEK_SET);
	}
      }
    }
  ...
  for (i = 0x21; i < 0x40; i++) {
    for (j = 0; j < 0x10; j++) {
      bmp_8x12[(i - 0x20) * 8 + j / 2][0x0c + (j % 2) * 2] = bmp_16x16[i][j] >> 8;
      bmp_8x12[(i - 0x20) * 8 + j / 2][0x0d + (j % 2) * 2] = bmp_16x16[i][j] & 0xff;
    }
  }
  for (i = 0x40; i < 0x80; i++) {
    for (j = 0; j < 0x10; j++) {
      bmp_8x8[(i - 0x40) * 4 + j / 4][0x08 + (j % 4) * 2] = bmp_16x16[i][j] >> 8;
      bmp_8x8[(i - 0x40) * 4 + j / 4][0x09 + (j % 4) * 2] = bmp_16x16[i][j] & 0xff;
    }
  }
  for (i = 0x90; i < 0xa0; i++) {
    for (j = 0; j < 0x10; j++) {
      bmp_8x10[0x100 + (i - 0x90) * 16 + j][0x0a] = bmp_16x16[i][j] >> 8;
      bmp_8x10[0x100 + (i - 0x90) * 16 + j][0x0b] = bmp_16x16[i][j] & 0xff;
    }
  }
  for (i = 0xb1; i < 0xc0; i++) {
    for (j = 0; j < 0x10; j++) {
      bmp_8x10[(i - 0xb0) * 16 + j][0x0a] = bmp_16x16[i][j] >> 8;
      bmp_8x10[(i - 0xb0) * 16 + j][0x0b] = bmp_16x16[i][j] & 0xff;
    }
  }
  for (i = 0xc0; i < 0xe0; i++) {
    for (j = 0; j < 0x10; j++) {
      bmp_8x10[(i - 0xc0) * 8 + j / 2][0x0c + (j % 2) * 2] = bmp_16x16[i][j] >> 8;
      bmp_8x10[(i - 0xc0) * 8 + j / 2][0x0d + (j % 2) * 2] = bmp_16x16[i][j] & 0xff;
    }
  }
  for (i = 0xe0; i < 0x100; i++) {
    for (j = 0; j < 0x10; j++) {
      bmp_8x10[0x100 + (i - 0xe0) * 8 + j / 2][0x0c + (j % 2) * 2] = bmp_16x16[i][j] >> 8;
      bmp_8x10[0x100 + (i - 0xe0) * 8 + j / 2][0x0d + (j % 2) * 2] = bmp_16x16[i][j] & 0xff;
    }
  }

  /* output */
  strncpy(outfile, "CGROM68.68", sizeof(outfile));
  ...
  for (i = 0; i < 0x100; i++) {
    for (j = 0; j < 0x10; j++) {
      fputc(bmp_8x12[i][j], fpout);
    }
  }
  for (i = 0; i < 0x100; i++) {
    for (j = 0; j < 0x10; j++) {
      fputc(bmp_8x8[i][j], fpout);
    }
  }
  for (i = 0; i < 0x200; i++) {
    for (j = 0; j < 0x10; j++) {
      fputc(bmp_8x10[i][j], fpout);
    }
  }
  fclose(fpout);
